...
import { faPrint, faChevronLeft, faCircleExclamation, faArrowLeft, faChevronRight, faTrash, faEnvelope } from '@fortawesome/free-solid-svg-icons'
...
const ImportantBodyRead = () => {
    ...
    //make spam
    const spam = async () => {
        await db.collection('receivers').doc(id.id).update({ isSpam: true }).then(res => toast({
            position: 'bottom-left',
    ...
    //make important
    const makeUnImportant = async () => {
        await db.collection('receivers').doc(id.id).update({ isImportant: false }).then(res => toast({
            position: 'bottom-left',
            description: "Conversation marked as unimportant !",
            status: 'success',
            duration: 3000,
            isClosable: true,
          }))
    ...
    return (
        <div className='mail__container'>
            ...
            <div style={body__left}>
                ...
                <Tooltip label="Mark as not important" aria-label='A tooltip'>
                    <Badge onClick={makeUnImportant} style={{ marginRight: '20px' }} colorScheme='red'><i className='fa fa-ban'></i></Badge>
                </Tooltip>
                ...
                <Tooltip label="Mask as spam" aria-label='A tooltip'>
                   <FontAwesomeIcon onClick={spam} style={{ marginRight: '30px', color: 'gray' }} icon={faCircleExclamation} />
                </Tooltip>
...
export default ImportantBodyRead
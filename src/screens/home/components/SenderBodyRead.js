...
import {changeStarredStateSender} from '../../../features/user/mailSlice'

const SenderBodyRead = () => {

    const selectedMail = useSelector((state) => state.composemail.senderSelectedMessage)
    const dispatch = useDispatch()
    const id = useParams();
    const toast = useToast();
    useEffect(() => {
        db.collection('senders').doc(id.id).update({ isRead: true }).then(res => console.log(res))
    }, [])

    //make starred
    const makeStarred = async()=>{
        await db.collection('senders').doc(id.id).update({ isStarred: true }).then(res => toast({
        ...
        dispatch(changeStarredStateSender(true))
    }
    //make un starred
    const makeUnStarred = async()=>{
        await db.collection('senders').doc(id.id).update({ isStarred: false }).then(res => toast({
            position: 'bottom-left',
        ...
        dispatch(changeStarredStateSender(false))
    }
    return (
        <div className='mail__container'>
            <ViewMailHeader mid={id.id} collection="senders" />

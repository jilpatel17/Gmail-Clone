...
import { openSenderMessage } from '../../../features/user/mailSlice'

const SenderList = ({isSpam,attachment,isImportant,isTrash,isRead,isStarred,mailid,profile,email,name,subject,message,time}) => {

    const navigate = useNavigate()
    const dispatch = useDispatch()
  
    const openMessageFroReadMail = async()=>{
        dispatch(openSenderMessage({
            profile,
            email,
            name,
            subject,
